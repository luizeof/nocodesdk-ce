                    ...
                    <div class="space-y-6">
                        @foreach ($this->user->tokens->sortBy('name') as $token)
                            <div class="flex items-center justify-between">
                                <div>
                                    <h3 class="text-sm text-slate-400">{{ $token->name }}</h3>
                                    <code class="font-mono text-xs text-slate-400">{{ $token->token_text }}</code>
                                </div>
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if (Laravel\Jetstream\Jetstream::hasPermissions())
                                        <button class="ml-6 text-sm text-gray-400 underline cursor-pointer" wire:click="manageApiTokenPermissions({{ $token->id }})">
                                            {{ __('Permissions') }}
                                        </button>
                                    @endif
